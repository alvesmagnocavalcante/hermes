...
DEFAULT_SERVICE_CODES = {
    "1000", "1003", "1004", "1005", "1022", "1122",  # diárias e ajustes
    "3010", "3015", "3115",  # upselling, toalha e ajuste
    "6000", "6003",  # lavanderia e avaria
    "9999",  # diárias de encerramento
}

TAIBA_SERVICE_CODES = {
    "1001", "1008", "1012", "1018", "1025", "2418", "3510", "5020", "5024",
    "6014", "6017", "6018", "6019", "6023", "9980",
}

CUMBUCO_SERVICE_CODES = {
    "1000", "1002", "1004", "1008", "1010", "1011", "1022", "1023", "1111",
    "3010", "3012", "3013", "3014", "3017", "5002", "5004", "5005", "5007",
    "5008", "5009", "5011", "6000", "9999",
}

CHARME_SERVICE_CODES = CUMBUCO_SERVICE_CODES | {
    "1001", "1009", "1019", "3011", "5081",
}
...
class RPSResult:
    ...
    def difference(self) -> Decimal | None:
        values = [value for value in (self.opera_value, self.fiscal_value, self.city_value) if value is not None]
...
def parse_date(value: Any) -> date | None:
    ...
    for pattern in ("%d/%m/%Y", "%d/%m/%y", "%d/%m/%Y %H:%M", "%d-%m-%Y", "%d-%m-%y", "%d-%b-%y", "%Y-%m-%d"):
        try:
    ...
    for table in parser.tables:
        for index, candidate in enumerate(table):
            ...
            if any(signature.issubset(keys) for signature in signatures):
                headers = candidate
                rows = [dict(zip(headers, row)) for row in table[index + 1:] if any(row)]
                return headers, rows
    raise ValueError(f"As colunas esperadas não foram encontradas no arquivo {path.name}.")
...
def identify_file(path: Path) -> str:
    ...
    if {"NUMERODOCUMENTO", "VALORDOCUMENTO", "SITUACAODOCUMENTO"}.issubset(keys):
        return "fiscal"
    if ({"NUMERORPS", "VALORDOSSERVICOS", "STATUSDOC"}.issubset(keys)
...
def read_opera(path: Path, service_codes: set[str] = DEFAULT_SERVICE_CODES) -> dict[str, OperaRPS]:
    result = {}
    for bill in ET.parse(path).getroot().findall(".//G_BILL_NO"):
        ...
        for transaction in bill.findall(".//G_TRX_NO"):
            ...
            description = re.sub(r"\s+", " ", transaction.findtext("TRANSACTION_DESCRIPTION") or "").strip()
            if description:
        ...
        if not rps or not issued:
            continue
        result[rps] = OperaRPS(rps, issued, str(bill.findtext("ROOM") or "").strip().zfill(4),
                               str(bill.findtext("DISPLAY_NAME") or "").strip(), abs(total).quantize(Decimal("0.01")),
...
def read_fiscal(path: Path) -> dict[str, FiscalRPS]:
    ...
    for row in rows:
        ...
        if not rps or rps == "NULL":
            continue
        result[rps] = FiscalRPS(rps, parse_date(row.get("DataEmissao")),
                                identifier(row.get("NumeroNFSe")),
                                str(row.get("NomeTomadorPrestador") or "").strip(),
                                decimal_value(row.get("ValorDocumento")).copy_abs().quantize(Decimal("0.01")),
                                str(row.get("SituacaoDocumento") or "").strip())
    return result


def read_city(path: Path, fiscal: dict[str, FiscalRPS] | None = None) -> dict[str, CityRPS]:
    _, rows = _sheet_rows(path)
    result = {}
    fiscal_by_nfse = {item.nfse: item.rps for item in (fiscal or {}).values() if item.nfse}
    for row in rows:
        values = {normalize(key): value for key, value in row.items()}
        rps = identifier(values.get("NUMERORPS") or values.get("NORPS") or values.get("NUMRPS"))
        ...
        result[key] = CityRPS(
            ...
            nfse,
            str(values.get("RAZAOSOCIALNOMEDOTOMADOR") or values.get("TOMADORDESERVICO") or values.get("NOMETOMADOR") or "").strip(),
            decimal_value(values.get("VALORDOSSERVICOS") or values.get("VALORDANFE") or values.get("VALORSERVICOS") or values.get("VALOR")).copy_abs().quantize(Decimal("0.01")),
            str(values.get("STATUSDOC") or values.get("SITUACAONOTA") or values.get("SITUACAO") or "Emitida").strip(),
        )
    return result
...
def analyze(paths: list[Path]) -> AnalysisResult:
    if len(paths) != 3:
        raise ValueError("Selecione um XML do Opera, uma planilha Fiscal e uma planilha da Prefeitura.")
    ...
    grouped = {kind: [path for path, current in identified if current == kind] for kind in ("opera", "fiscal", "city")}
    if any(len(items) != 1 for items in grouped.values()):
        raise ValueError("Envie exatamente um arquivo de cada fonte: Opera, Fiscal e Prefeitura.")
    ...
    def profile_score(rows: dict[str, OperaRPS]) -> tuple[int, int, int]:
        ...
        exact = sum(abs(rows[rps].value - fiscal[rps].value) <= Decimal("0.01") for rps in shared)
        return exact, len(shared), len(rows)

    service_profile, opera = max(candidates.items(), key=lambda item: profile_score(item[1]))
    fiscal_dates = [item.issue_date for item in fiscal.values() if item.issue_date]
    if not opera:
        raise ValueError("O XML do Opera não contém RPS de serviços reconhecidos para conferência.")
    if not fiscal:
        raise ValueError("A planilha Fiscal não contém RPS utilizáveis para conferência.")
    if not city:
        raise ValueError("A planilha da Prefeitura não contém RPS utilizáveis para conferência.")
    if not fiscal_dates:
    ...
    for rps in rps_numbers:
        ...
        customer = (op.guest if op else "") or (fi.customer if fi else "") or (ci.customer if ci else "")
        if fi and "CANCEL" in normalize(fi.status) and ci and "CANCEL" in normalize(ci.status):
            status, detail = "Cancelado", "RPS cancelado tanto no Fiscal quanto na Prefeitura."
        elif fi and "IRREGULAR" in normalize(fi.status):
            status, detail = "Fiscal irregular", "O RPS integrou, mas está inválido/irregular e não possui nota válida na Prefeitura."
        elif op and op.issue_date > fiscal_end and not fi:
            status = "Fora do período do Fiscal"
            detail = (f"Não avaliado: este RPS é de {op.issue_date:%d/%m/%Y}, mas o relatório Fiscal "
                      f"selecionado contém informações somente até {fiscal_end:%d/%m/%Y}. ")
        elif not op:
            status, detail = "Ausente no Opera", "RPS localizado nas fontes fiscais, mas não no XML de encerramentos do Opera."
        elif not fi and not ci:
            status, detail = "Ausente Fiscal/Prefeitura", "RPS de serviço encerrado no Opera sem integração Fiscal e sem NFS-e na Prefeitura."
        elif not fi:
            status, detail = "Ausente no Fiscal", "RPS não localizado no módulo Fiscal do CMFlex."
        elif not ci:
            status, detail = "Ausente na Prefeitura", "RPS integrado no Fiscal, mas sem NFS-e correspondente na Prefeitura."
        elif max(values) - min(values) > Decimal("0.01"):
            status, detail = "Valor divergente", "Os valores de serviço não coincidem nas três fontes."
        else:
            status, detail = "Conciliado", "RPS e valor localizados no Opera, Fiscal e Prefeitura."
        results.append(RPSResult(rps, op.issue_date if op else None, op.room if op else "", customer,
                                 op.services if op else "", op.value if op else None, fi.value if fi else None,
                                 ci.value if ci else None, fi.nfse if fi else "", ci.nfse if ci else "",
                                 fi.status if fi else "Ausente", ci.status if ci else "Ausente", status, detail))
    results.sort(key=lambda row: (row.reconciled, row.incomplete, row.cancelled, row.opera_date or date.min, row.rps))
    return AnalysisResult(results, fiscal_start, fiscal_end, tuple(path.name for path in paths), service_profile)
...
def save_excel(result: AnalysisResult, path: Path) -> None:
    ...
    for label, value in (
        ("Período coberto pelo Fiscal", f"{result.fiscal_start:%d/%m/%Y} a {result.fiscal_end:%d/%m/%Y}"),
        ("Perfil de serviços identificado", result.service_profile),
        ("RPS analisados", len(result.rows)), ("Conciliados", result.reconciled),
        ("Pendências", result.issues), ("Fora do período", result.incomplete), ("Cancelados", result.cancelled),
    ):
    ...
    detail.append(["RPS", "Data Opera", "Hóspede/Tomador", "Serviços Opera", "Valor Opera",
                   "Valor Fiscal", "Valor Prefeitura", "Diferença", "NFS-e Fiscal", "NFS-e Prefeitura",
                   "Situação Fiscal", "Situação Prefeitura", "Resultado", "Explicação"])
    for row in result.rows:
        detail.append([row.rps, row.opera_date, row.customer, row.services,
                       ...
                       None if row.difference is None else float(row.difference), row.fiscal_nfse, row.city_nfse,
                       row.fiscal_status, row.city_status, row.status, row.detail])
    for cell in detail["B"][1:]:
        cell.number_format = "dd/mm/yyyy"
    for column in ("E", "F", "G", "H"):
        for cell in detail[column][1:]:
            cell.number_format = 'R$ #,##0.00'
    for column, width in {"A": 13, "B": 14, "C": 34, "D": 40, "E": 16, "F": 16,
                          "G": 18, "H": 16, "I": 15, "J": 18, "K": 25, "L": 24, "M": 28, "N": 70}.items():
...
def save_pdf(result: AnalysisResult, path: Path) -> None:
    ...
    data = [["RPS", "Conciliados", "Pendências", "Fora do período", "Cancelados"],
            [str(len(result.rows)), str(result.reconciled), str(result.issues), str(result.incomplete), str(result.cancelled)]]
    ...
    details = [["RPS", "Data", "Hóspede/Tomador", "Opera", "Fiscal", "Prefeitura", "Resultado"]]
    details.extend([[row.rps, row.opera_date.strftime("%d/%m/%Y") if row.opera_date else "—", row.customer[:30],
                     money(row.opera_value), money(row.fiscal_value), money(row.city_value), row.status] for row in pending])
    detail_table = Table(details, colWidths=[22 * mm, 25 * mm, 65 * mm, 30 * mm, 30 * mm, 32 * mm, 48 * mm], repeatRows=1)
    detail_table.setStyle(TableStyle([("BACKGROUND", (0, 0), (-1, 0), colors.HexColor("#374151")),
                                      ("TEXTCOLOR", (0, 0), (-1, 0), colors.white),
                                      ("FONTSIZE", (0, 0), (-1, -1), 8),
                                      ("GRID", (0, 0), (-1, -1), .4, colors.grey)]))
    story = [Paragraph("RPS de serviços prestados — Opera x Fiscal x Prefeitura", styles["Title"]),
             Paragraph(f"Perfil identificado: {result.service_profile} • Fiscal de {result.fiscal_start:%d/%m/%Y} a {result.fiscal_end:%d/%m/%Y}", styles["BodyText"]),
             Spacer(1, 5 * mm), table, Spacer(1, 6 * mm), Paragraph("Registros que exigem atenção", styles["Heading2"]), detail_table]
    SimpleDocTemplate(str(path), pagesize=landscape(A4), title="Conferência de RPS").build(story)


class ProvidedServicesRPSAutomation(Automation):
    name = "RPS de Serviços Prestados"
    ...
    def render(self):
        ...
        ctk.CTkLabel(self.container, text="Confere se os RPS encerrados no Opera integraram no Fiscal do CMFlex e foram emitidos na Prefeitura.", text_color="gray70").grid(row=1, column=0, padx=30, pady=(0, 10), sticky="w")
        controls = ctk.CTkFrame(self.container, fg_color="transparent")
        controls.grid(row=2, column=0, padx=30, sticky="ew")
        self.select = ctk.CTkButton(controls, text="Selecionar Opera, Fiscal e Prefeitura", command=self._select)
        ...
        self.info = ctk.CTkLabel(self.container, text="Selecione o XML do Opera e as planilhas Fiscal e Prefeitura; os nomes podem variar.", text_color="gray70", anchor="w")
        ...
        self.cards = {}
        for index, title in enumerate(("RPS analisados", "Conciliados", "Pendências", "Fora do período")):
        ...
        ctk.CTkLabel(chart_frame, text="Distribuição da integração", font=ctk.CTkFont(size=13, weight="bold")).pack(pady=(6, 0))
        ...
        entry = ctk.CTkEntry(filters, textvariable=self.search, placeholder_text="RPS, NFS-e, hóspede, serviço ou resultado")
        entry.grid(row=0, column=2, sticky="ew")
        entry.bind("<KeyRelease>", lambda _: self._reset())
        ctk.CTkLabel(filters, text="Verde: integrado nas três fontes  •  Vermelho: ausência/valor incorreto  •  Amarelo: cancelado ou período incompleto", text_color="gray70", anchor="w").grid(row=1, column=0, columnspan=3, pady=(6, 0), sticky="ew")

        self.preview = create_result_table(self.container, (
            TableColumn("rps", "RPS", 90), TableColumn("date", "Data Opera", 105),
            TableColumn("customer", "Hóspede/Tomador", 250), TableColumn("opera", "Opera", 120, "e"),
            TableColumn("fiscal", "Fiscal", 120, "e"), TableColumn("city", "Prefeitura", 120, "e"),
            TableColumn("nfse", "NFS-e", 100), TableColumn("difference", "Diferença", 120, "e"),
            TableColumn("status", "Resultado", 210),
        ), row=6)
    ...
    def _select(self):
        ...
        self.app.set_status("Conferindo RPS nas três fontes...", .1)
    ...
    def _done(self, result):
        ...
        self.info.configure(text=f"Hotel identificado: {result.service_profile} • Fiscal de {result.fiscal_start:%d/%m/%Y} a {result.fiscal_end:%d/%m/%Y} • {len(result.rows)} RPS • {result.cancelled} cancelado(s)")
        self.app.set_status("Conferência de RPS concluída", 1)
        self._show()
    ...
    def _filtered(self):
        ...
        return [row for row in self.result.rows
                if (status == "Todos" or status == "Conciliados" and row.reconciled or status == "Pendências" and not row.reconciled)
                and (not search or search in normalize(row.rps + row.room + row.customer + row.services + row.fiscal_nfse + row.city_nfse + row.status + row.detail))]
    ...
    def _show(self):
        ...
        if not self.result:
            ...
            values = (str(len(self.result.rows)), str(self.result.reconciled), str(self.result.issues), str(self.result.incomplete))
            ...
            for row in filtered[self.page * self.page_size:(self.page + 1) * self.page_size]:
                tag = "ok" if row.reconciled else "missing" if row.incomplete or row.cancelled else "error"
                self.preview.insert("", "end", values=(row.rps, row.opera_date.strftime("%d/%m/%Y") if row.opera_date else "—",
                                    row.customer, money(row.opera_value), money(row.fiscal_value), money(row.city_value),
                                    row.city_nfse or row.fiscal_nfse or "—", money(row.difference), row.status), tags=(tag,))
            self.page_label.configure(text=f"Página {self.page + 1} de {pages} • {len(filtered)} RPS")
    ...
    def _draw_chart(self):
        ...
        if not self.result:
            ...
            return
        total = len(self.result.rows) or 1
        ok_ratio = self.result.reconciled / total
        info_ratio = (self.result.incomplete + self.result.cancelled) / total
        ...
        if info_ratio:
            self.chart.create_arc(left, top, left + size, top + size, start=90 - 360 * ok_ratio,
                                  extent=-360 * info_ratio, style="arc", outline="#e0a83e", width=13)
    ...
    def _export(self):
        ...
        name = filedialog.asksaveasfilename(title="Exportar conferência", defaultextension=extension,
                                            initialfile=f"conferencia_rps_{self.result.fiscal_end:%Y_%m_%d}{extension}",
                                            filetypes=[(self.output_format.get(), f"*{extension}")])
        if not name:
            return
        result = self.result
        task = save_excel if extension == ".xlsx" else save_pdf
        self.app.set_status("Exportando conferência de RPS...", .5)
    ...
    def _clear(self):
        ...
        self.info.configure(text="Selecione o XML do Opera e as planilhas Fiscal e Prefeitura; os nomes podem variar.")
        self._show()
        self.app.set_status("Seleção limpa", 0)


AUTOMATION_CLASS = ProvidedServicesRPSAutomation
