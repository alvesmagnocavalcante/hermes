...
HEADERS = ["Hotel vendedor", "Empresa compradora", "Nº da nota", "Data de emissão", "Item", "Valor"]
STOP_WORDS = (
    "TOTAL LOCACAO", "DADOS PARA DEPOSITO", "NO CASO DE PAGAMENTO", "BANCO",
    "AGENCIA", "CONTA", "PIX", "CNPJ", "OBSERVACAO", "FORMA DE PAGAMENTO", "OPERACAO CC",
)
...
class ReportRow:
    ...
    def values(self) -> list[Any]:
        return [self.hotel, self.comprador, self.nota, self.emissao, self.item, self.valor]
...
def find_header(sheet: Worksheet) -> int | None:
    for row in sheet.iter_rows(min_row=1, max_row=min(sheet.max_row, 80)):
        ...
        if any(label in {"OBJETO", "DESCRICAO", "DESCRICAO ITEM", "LEITURA", "ITEM"} for label in labels) and any(
            "V.TOTAL" in label or "VALOR TOTAL" in label for label in labels
        ):
            return row[0].row
    return None
...
def choose_column(headers: dict[str, int], candidates: tuple[str, ...]) -> int | None:
    ...
    return next((column for header, column in headers.items() if any(c in header for c in candidates)), None)


def seller(sheet: Worksheet, source: Path) -> str:
    for row in sheet.iter_rows(min_row=1, max_row=min(sheet.max_row, 8), max_col=min(sheet.max_column, 6)):
        for cell in row:
            text, normalized = clean(cell.value), normalize(cell.value)
            invalid = (
                not text or normalized in {"#VALUE!", "#VALOR!"} or "@" in text
                or normalized.startswith(("WWW.", "RUA ", "AV ", "AV:", "AVENIDA ", "RODOVIA "))
...
def parse_note_number(value: Any, sheet_name: str) -> str:
    text = clean(value)
    match = re.search(r"(?:N[°Oº.]?\s*)?([0-9]+(?:[\s./-]*[0-9]+)*)", text, re.IGNORECASE)
    if match:
        return re.sub(r"\s+", "", match.group(1))

    sheet_match = re.search(r"NF\s*([0-9]+(?:[\s./-]*[0-9]+)*)", sheet_name, re.IGNORECASE)
...
def extract_sheet(sheet: Worksheet, source: Path) -> list[ReportRow]:
    ...
    if header_row is None:
        return []
    headers = {normalize(cell.value): cell.column for cell in sheet[header_row] if normalize(cell.value)}
    item_col = choose_column(headers, ("OBJETO", "DESCRICAO", "DESCRICAO ITEM", "LEITURA", "ITEM"))
    value_col = choose_column(headers, ("V.TOTAL", "VALOR TOTAL", "VALOR TOTAL DO REPASSE", "VALOR TOTAL NOTA"))
    ...
    for number in range(header_row + 1, sheet.max_row + 1):
        beginning = " ".join(normalize(sheet.cell(number, column).value) for column in range(1, 4))
        if any(word in beginning for word in STOP_WORDS):
            break
        item, value = clean(sheet.cell(number, item_col).value), money(sheet.cell(number, value_col).value)
...
def save_excel(rows: list[ReportRow], path: Path) -> None:
    ...
    for cells in sheet.columns:
        sheet.column_dimensions[cells[0].column_letter].width = min(max(max(len(clean(c.value)) for c in cells) + 2, 12), 60)
...
def save_pdf(rows: list[ReportRow], path: Path) -> None:
    ...
    document = SimpleDocTemplate(str(path), pagesize=landscape(A4), leftMargin=8 * mm, rightMargin=8 * mm, topMargin=10 * mm, bottomMargin=10 * mm)
    table = Table(data, repeatRows=1, colWidths=[36 * mm, 42 * mm, 22 * mm, 27 * mm, 105 * mm, 28 * mm])
...
class DebitReportAutomation(Automation):
    name = "Relatório de Notas de Débito"
    ...
    def render(self) -> None:
        ...
        self.summary = ctk.CTkLabel(self.container, text="Selecione uma ou mais planilhas.", text_color="gray70")
        self.summary.grid(row=1, column=0, padx=30, pady=(0, 10), sticky="w")

        controls = ctk.CTkFrame(self.container, fg_color="transparent")
        controls.grid(row=2, column=0, padx=30, sticky="ew")
        self.select_button = ctk.CTkButton(controls, text="Selecionar planilhas", command=self._select_files)
        ...
        self.generate_button = ctk.CTkButton(controls, text="Gerar relatório", state="disabled", command=self._generate)
        ...
        self.preview = create_result_table(self.container, (
            TableColumn("hotel", "Hotel vendedor", 230), TableColumn("buyer", "Empresa compradora", 260),
            TableColumn("note", "Nota", 105), TableColumn("date", "Emissão", 95),
            TableColumn("item", "Item", 390), TableColumn("value", "Valor", 130, "e"),
        ), row=3, pady=(10,14))
        self._show_preview()
    ...
    def _select_files(self) -> None:
        ...
        self._set_busy(True)
        self.app.set_status("Lendo planilhas...", 0.1)
        self.app.run_background(lambda: extract(self.files), self._preview_ready, self._task_failed)

    def _preview_ready(self, rows: list[ReportRow]) -> None:
        self.rows = rows
        self._set_busy(False)
        self._show_preview()
        self.app.set_status(f"{len(rows)} itens encontrados", 1)

    def _show_preview(self) -> None:
        clear_table(self.preview)
        for row in self.rows:
            self.preview.insert("", "end", values=(row.hotel, row.comprador, row.nota, row.emissao,
                row.item, format_currency(row.valor)))
        self.summary.configure(text=f"{len(self.files)} planilha(s) • {len(self.rows)} item(ns)")
        self.generate_button.configure(state="normal" if self.rows else "disabled")

    def _generate(self) -> None:
        output_format = self.output_format.get()
        extension = ".pdf" if output_format == "PDF" else ".xlsx"
        path = filedialog.asksaveasfilename(title="Salvar relatório", defaultextension=extension, initialfile=f"relatorio_notas_debito{extension}", filetypes=[(output_format, f"*{extension}")])
        if not path:
            return
        self._set_busy(True)
        self.app.set_status(f"Gerando {output_format}...", 0.2)
        task = lambda: save_pdf(self.rows, Path(path)) if output_format == "PDF" else save_excel(self.rows, Path(path))
        self.app.run_background(task, lambda _: self._saved(Path(path)), self._task_failed)

    def _saved(self, path: Path) -> None:
        self._set_busy(False)
        self.app.set_status("Relatório gerado", 1)
        messagebox.showinfo("Relatório gerado", f"Arquivo salvo em:\n{path}\n\nItens exportados: {len(self.rows)}")
    ...
    def _set_busy(self, busy: bool) -> None:
        ...
        self.generate_button.configure(state="disabled" if busy or not self.rows else "normal")


AUTOMATION_CLASS = DebitReportAutomation
