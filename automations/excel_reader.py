...
def _html_workbook(content: bytes, source: Path) -> Workbook:
    ...
    if not parser.tables:
        ...
        raise ValueError(
            f"{source.name}: nenhuma tabela foi encontrada no arquivo .xls."
        )
    tables = sorted(
        parser.tables,
...
def _xls_workbook(content: bytes, source: Path) -> Workbook:
    try:
        ...
        raise ValueError(
            f"{source.name}: arquivo .xls inválido ou corrompido."
        ) from error
    workbook = Workbook()
    workbook.remove(workbook.active)
    try:
            ...
            for row_index in range(source_sheet.nrows):
                ...
                for column_index in range(source_sheet.ncols):
                    ...
                    if cell.ctype == xlrd.XL_CELL_DATE:
                        ...
                        cell.ctype == xlrd.XL_CELL_NUMBER and float(value).is_integer()
                    ):
                        value = int(value)
    ...
    if not workbook.worksheets:
        raise ValueError(
            f"{source.name}: nenhuma planilha foi encontrada no arquivo .xls."
        )
    return workbook
